// update.js — 自用签到 自更新模块
//
// 半自动更新流程（方案 2，2026-09-17 设计）：
//   1. checkUpdate()   — 请求 GitHub API 查询本仓库最新 Release（checkin/vX.Y.Z）
...
const CURRENT_VERSION = process.env.CHECKIN_VERSION || "1.0.11";

// 更新目录：应用数据目录 updates/
function updateDir(dataDir) {
  return path.join(dataDir || "/vol1/@appdata/checkin", "updates");
}
...
function httpsGetJson(url, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(
      u,
      { headers: { "User-Agent": "checkin-updater/1.0", Accept: "application/vnd.github+json" }, timeout: timeoutMs },
      (res) => {
        let data = "";
...
/** 解析 GitHub Releases，返回最新的 checkin release 信息。 */
function parseLatestRelease(releases) {
  ...
  for (const r of releases) {
    ...
    if (!tag.startsWith("checkin/v")) continue;
    const version = tag.replace("checkin/v", "");
...
function parseRawManifest(m) {
  ...
  const file = String(m.file || `checkin_${version}_all.fpk`);
  return {
    version,
    tag: String(m.tag || `checkin/v${version}`),
    ...
    html_url: `https://github.com/LittlePigeno217/fnos-apps/releases/tag/checkin/v${version}`,
    body: "",
  };
}
...
function downloadUpdate(payload, dataDir) {
  return new Promise((resolve) => {
    ...
    const expectName = `checkin_${String((payload && payload.version) || "")}_all.fpk`;
    ...
    const headers = { "User-Agent": "checkin-updater/1.0" };
...
function downloadFollow(tmpPath, filePath, targetUrl, name, expected, expectedSha, resolve) {
  const u = new URL(targetUrl);
  const mod = u.protocol === "http:" ? http : https;
  const req = mod.get(u, { headers: { "User-Agent": "checkin-updater/1.0" }, timeout: 60000 }, (res) => {
    if (res.statusCode && res.statusCode >= 400) {
      resolve({ success: false, message: `下载失败: HTTP ${res.statusCode}` });
      res.resume();
      return;
...
function applyUpdate(payload, dataDir, creds) {
  return new Promise(async (resolve) => {
    ...
    if (!/^checkin_[0-9a-zA-Z._-]+\.fpk$/.test(name)) {
      resolve({ success: false, message: "无效的 FPK 文件名" });
      return;
    }
    ...
    if (cls.need_ui) {
      resolve({
        ...
        steps: ["打开「应用中心」→「已安装」→ 自用签到", "点击「手动安装」，选择上一步下载的安装包", "同意并确认，应用自动升级"],
      });
      return;
    }
