...
${CHANGELOG}
**国内镜像**:
- [${FILE_PREFIX}_${VERSION}_x86.fpk](https://ghfast.top/https://github.com/LittlePigeno217/fnos-apps/releases/download/${RELEASE_TAG}/${FILE_PREFIX}_${VERSION}_x86.fpk)
- [${FILE_PREFIX}_${VERSION}_arm.fpk](https://ghfast.top/https://github.com/LittlePigeno217/fnos-apps/releases/download/${RELEASE_TAG}/${FILE_PREFIX}_${VERSION}_arm.fpk)
EOF
...
info " 11. Run scripts/test/static-check.sh $APPNAME      # L1: static scaffold validation"
info ""
info "After implementing and a successful local build (dist/<slug>_*.fpk):"
info " 12. Run scripts/test/verify-fpk.sh dist/<fpk>           # L2: built-artefact contract check"
info " 13. Run scripts/test/run-fpk-tests.sh dist/<fpk>        # L3: install → start → probe → stop → uninstall"
info ""
info "When a second version is built (or you want to validate upgrade hooks before release):"
info " 14. Run scripts/test/run-fpk-tests.sh --upgrade-from dist/<old.fpk> dist/<new.fpk>"
info "     L3 upgrade: install OLD → start → stop → upgrade NEW → start → probe → stop → uninstall"
info "     Verifies service_preupgrade/save/restore/postupgrade hooks AND user-data persistence."
