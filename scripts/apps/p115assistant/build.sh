...
# p115assistant 打包脚本：生成 app.tgz（server + ui 运行时）。
...
APP_DIR="${SCRIPT_DIR}/../../../apps/p115assistant"
...
echo "[p115assistant] 打包版本: ${VERSION}"
...
cat > "${WORK_DIR}/config/bootstrap/p115assistant-version.env" <<EOF
# 115网盘助手 目标版本（构建时注入；upgrade_callback 读取用于版本记录/检测）
P115ASSISTANT_VERSION=${VERSION}
EOF
echo "[p115assistant] 版本 env 已写入: config/bootstrap/p115assistant-version.env"
...
echo "[p115assistant] 版本注入完成: ${VERSION}"
...
if command -v node >/dev/null 2>&1; then
    ...
    echo "[p115assistant] Node 语法自检通过"
fi

cd "${WORK_DIR}"
tar czf "${REPO_ROOT}/app.tgz" server/ www/ ui/ config/
echo "[p115assistant] 已生成 app.tgz（$(du -h "${REPO_ROOT}/app.tgz" | cut -f1)）"
